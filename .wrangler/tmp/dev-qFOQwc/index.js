...
${normalizedNotes.map(
    ...
    if (method === "GET" && path2 === "/persona/teamState") {
      ...
      if (!team) return json({ error: "missing_team" }, 400);
      const data = await env.TEAM_STATE.get(team, "json");
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(json(data || {}, 200, h2), method, path2, rawPath);
    }
...
// .wrangler/tmp/bundle-nBgRaq/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-nBgRaq/middleware-loader.entry.ts
