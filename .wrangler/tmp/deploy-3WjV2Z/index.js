...
var index_default = {
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === "GET" && pathname === "/persona/checkKey") {
      ...
      const masked = exists ? env.OPENAI_API_KEY.slice(0, 4) + "****" + env.OPENAI_API_KEY.slice(-4) : "(none)";
      const res = json({ exists, key: masked }, 200, h2);
      return addEngineHeaders(corsResponse(await res.text(), 200, h2, origin), method, pathname);
    }
