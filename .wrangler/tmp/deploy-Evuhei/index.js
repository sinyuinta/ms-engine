...
function allowedOrigin(env, req) {
  ...
  const allowed = [
    "http://localhost:5173",
    "https://shiko-athletic-test.pages.dev"
  ];
  return allowed.includes(origin) ? origin : "https://shiko-athletic-test.pages.dev";
}
...
${normalizedNotes.map(
    ...
    if (method === "POST" && path2 === "/persona/login-request") {
      ...
      await env.TEAM_STATE.put(`oauth_state_${state}`, JSON.stringify({ redirect }), {
        expirationTtl: 300
      });
      ...
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(json({ ok: true, url: url2 }), method, path2, rawPath);
    }
    if (method === "GET" && path2 === "/persona/oauth-callback") {
      ...
      await env.TEAM_STATE.put(userKey, JSON.stringify({ email, name }));
