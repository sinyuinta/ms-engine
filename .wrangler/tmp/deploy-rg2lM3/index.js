...
__name(requireCompany, "requireCompany");
...
\u51FA\u529B\u306FJSON\u914D\u5217\u306E\u307F\u3067\u3001\u65E5\u672C\u8A9E\u306E\u540D\u8A5E\u53E5\u30678\u301C12\u4EF6\u7A0B\u5EA6\u3002
    ...
    if (method === "POST" && path2 === "/auth/companyLogin") {
      const h2 = withCors(new Headers(), origin);
      try {
        const { companyCode, password } = await req.json();
        if (!companyCode || !password) {
          return addEngineHeaders(
            json({ ok: false, error: "MISSING_PARAMS" }, 400, h2),
            method,
            path2,
            rawPath
          );
        ...
        if (!record || record.enabled !== true) {
          return addEngineHeaders(
            json({ ok: false, error: "INVALID_COMPANY" }, 401, h2),
            method,
            path2,
            rawPath
          );
        ...
        if (record.expiresAt && Date.now() > Date.parse(record.expiresAt)) {
          ...
          return addEngineHeaders(
            json({ ok: false, error: "EXPIRED" }, 403, h2),
            method,
            path2,
            rawPath
          );
        ...
        if (hashed !== record.passwordHash) {
          return addEngineHeaders(
            json({ ok: false, error: "INVALID_PASSWORD" }, 401, h2),
            method,
            path2,
            rawPath
          );
        ...
        return addEngineHeaders(
          json({ ok: true, companyName: record.companyName }, 200, h2),
          method,
          path2,
          rawPath
        );
        ...
        return addEngineHeaders(
          json({ ok: false, error: "LOGIN_FAILED" }, 500, h2),
          method,
          path2,
          rawPath
        );
    ...
    if (method === "POST" && path2 === "/auth/companyPing") {
      const h2 = withCors(new Headers(), origin);
      try {
        const { companyCode } = await req.json();
        if (!companyCode) {
          return addEngineHeaders(
            json({ ok: false }, 401, h2),
            method,
            path2,
            rawPath
          );
        ...
        if (!record || record.enabled !== true || record.expiresAt && Date.now() > Date.parse(record.expiresAt)) {
          return addEngineHeaders(
            json({ ok: false }, 401, h2),
            method,
            path2,
...
${normalizedNotes.map(
    ...
    if (method === "DELETE" && path2 === "/persona/teamState") {
      const h2 = withCors(new Headers(), origin);
      const companyCode = url.searchParams.get("companyCode");
      const team = url.searchParams.get("team");
      if (!companyCode) {
        return addEngineHeaders(
          json({ ok: false, error: "companyCode is required" }, 401, h2),
          method,
          path2,
          rawPath
        );
      ...
      if (!team) {
        return addEngineHeaders(
          json({ ok: false, error: "team is required" }, 400, h2),
          method,
          path2,
          rawPath
        );
      ...
      console.log(`\u{1F5D1} team deleted: ${companyCode}/${team}, keys=${list.keys.length}`);
      return addEngineHeaders(
        json(
          {
            ok: true,
    ...
    if (method === "POST" && path2 === "/team/updateMembers") {
      const h2 = withCors(new Headers(), origin);
      try {
        const body = await req.json();
        const companyCode = requireCompany(body);
        ...
        if (!team || !Array.isArray(members)) {
          return addEngineHeaders(
            json({ error: "invalid_args" }, 400, h2),
            method,
            path2,
            rawPath
          );
        ...
        const meta = rawMeta ? JSON.parse(rawMeta) : { team, users: [], roles: {} };
        meta.users = members;
        ...
        return addEngineHeaders(
          json({ ok: true, team, members }, 200, h2),
          method,
          path2,
          rawPath
        );
    ...
    if (method === "POST" && path2 === "/admin/issueCompany") {
      const h2 = withCors(new Headers(), origin);
      try {
        const body = await req.json();
        const {
        ...
        if (!companyCode || !password) {
          return addEngineHeaders(
            json({ error: "missing_params" }, 400, h2),
            method,
            path2,
            rawPath
          );
        ...
        return addEngineHeaders(
          json({ ok: true, companyCode }, 200, h2),
          method,
          path2,
          rawPath
        );
        ...
        return addEngineHeaders(
          json({ error: "issue_failed" }, 500, h2),
          method,
          path2,
          rawPath
        );
