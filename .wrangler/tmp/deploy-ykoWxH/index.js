...
${text2}`
    ...
    if (method === "POST" && path2 === "/persona/noise") {
      ...
      try {
        ...
        const textForAI = Array.isArray(note?.plans) ? note.plans.map((p, i) => `(${i + 1}) ${p.who || "\u8AB0\u304B"}\u306E\u8A08\u753B`).join("\n") : typeof note === "string" ? note : JSON.stringify(note ?? {}, null, 2);
        const client = new OpenAI({ apiKey: env.OPENAI_API_KEY });
        const completion = await client.chat.completions.create({
          model: "gpt-4o-mini",
          temperature: 0.4,
          messages: [
            { role: "system", content: "JSON\u30674\u8996\u70B9\u3092\u51FA\u529B\u305B\u3088" },
            { role: "user", content: `\u8B70\u984C:${topic}
${textForAI}` }
          ]
        });
        ...
        return addEngineHeaders(
          json({ ok: true, perspectives: parsed }, 200, h2),
          method,
          path2,
          rawPath
        );
        ...
        return addEngineHeaders(
          json({ error: "noise_failed", message: String(err) }, 500, h2),
          method,
          path2,
          rawPath
        );
