...
var src_default = {
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === "OPTIONS" && path2 === "/persona/teamUserStates") {
      const h2 = withCors(new Headers(), origin);
    ...
    if (method === "GET" && path2 === "/persona/teamUserStates") {
      ...
      if (!team) {
        return addEngineHeaders(
          json({ error: "missing_team" }, 400, h2),
          method,
          path2,
          rawPath
        );
      ...
      for (const key of list.keys) {
        const data = await env.TEAM_STATE.get(key.name, "json");
        if (data) {
          users.push({
            ...data,
            userKey: key.name
...
// .wrangler/tmp/bundle-KngLu1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-KngLu1/middleware-loader.entry.ts
