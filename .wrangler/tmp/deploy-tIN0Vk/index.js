...
function withCors(base, origin) {
  if (origin) base.set("Access-Control-Allow-Origin", origin);
...
var index_default = {
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === "OPTIONS" && path2.startsWith("/persona/")) {
      const h2 = withCors(new Headers(), allowedOrigin(env, req));
    ...
    if (method === "OPTIONS" && path2 === "/persona/targets") {
      const h2 = withCors(new Headers(), allowedOrigin(env, req));
      return addEngineHeaders(
        new Response(null, { status: 204, headers: h2 }),
        method,
