...
${normalizedNotes.map(
    ...
    if (method === "POST" && path2 === "/auth/companyRegister") {
      const h2 = withCors(new Headers(), origin);
      try {
        const { companyCode, companyName, password } = await req.json();
        if (!companyCode || !password) {
          return addEngineHeaders(
        ...
        const key = `COMPANY:${companyCode}`;
        const exists = await env.COMPANY_AUTH.get(key);
        if (exists) {
          return addEngineHeaders(
            json({ error: "ALREADY_EXISTS" }, 409, h2),
            method,
            path2,
            rawPath
          );
        ...
        return addEngineHeaders(
          json({ error: "REGISTER_FAILED" }, 500, h2),
          method,
          path2,
          rawPath
        );
