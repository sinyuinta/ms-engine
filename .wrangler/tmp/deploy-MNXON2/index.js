...
// src/index.ts
var proxyUrl = "https://proud-koala-28-8qvxvh0jdsjt.deno.dev";
var BUILD = "cors-v3-2025-09-14-p2";
...
function withCors(base, origin) {
  base.set("Access-Control-Allow-Origin", origin);
  base.set("Access-Control-Allow-Methods", "GET,POST,OPTIONS,HEAD");
  base.set(
    "Access-Control-Allow-Headers",
    "content-type, cache-control, x-otb-salt, authorization"
  );
  base.append("Vary", "Origin, Access-Control-Request-Method, Access-Control-Request-Headers");
  base.set("Cache-Control", "no-store");
  return base;
}
__name(withCors, "withCors");
function addEngineHeaders(res, method, path2, rawPath, target) {
  const h = new Headers(res.headers);
  h.set("x-engine-build", BUILD);
  h.set("x-engine-method", method);
  h.set("x-engine-path", path2);
...
__name(json, "json");
function text(body = "ok", status = 200, headers) {
  const h = new Headers(headers);
...
var index_default = {
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === "OPTIONS") {
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(new Response(null, { status: 204, headers: h2 }), method, path2, rawPath);
    }
    if (method === "GET" && (path2 === "/health" || path2 === "/persona/health")) {
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(text("ok", 200, h2), method, "/health", rawPath);
    }
    await env.TEAM_STATE.put(teamKey, JSON.stringify(teamData));
    const logKey = `logs_${teamName}`;
    const logs = await env.TEAM_STATE.get(logKey, { type: "json" }) || [];
    logs.push({
      ...
      premise: teamData.premise,
      trouble: teamData.trouble,
      otherPrem: teamData.otherPrem,
      cause: teamData.cause,
      idea: teamData.idea,
      plans: teamData.plans,
      createdAt: (/* @__PURE__ */ new Date()).toISOString()
    });
    await env.TEAM_STATE.put(logKey, JSON.stringify(logs));
    if (method === "GET" && path2 === "/persona/teamLogs") {
      const teamName2 = url.searchParams.get("team");
      if (!teamName2) {
        return json([], 400);
      }
      const logKey2 = `logs_${teamName2}`;
      const logs2 = await env.TEAM_STATE.get(logKey2, { type: "json" }) || [];
      return json(logs2);
    }
    if ((method === "GET" || method === "POST") && path2 === "/persona/targets") {
      const h2 = withCors(new Headers(), origin);
      try {
        ...
        if (!topic.trim()) {
          return addEngineHeaders(
            json({ ok: true, topic: "", targets: [] }, 200, h2),
            method,
            path2,
            rawPath
          );
...
\u51FA\u529B\u306FJSON\u914D\u5217\u306E\u307F\u3067\u3001\u65E5\u672C\u8A9E\u306E\u540D\u8A5E\u53E5\u30678\u301C12\u4EF6\u7A0B\u5EA6\u3002
      `
        ...
        return addEngineHeaders(
          json({ ok: true, topic, targets: parsed }, 200, h2),
          method,
          path2,
          rawPath
        );
    ...
    if (method === "HEAD" && path2 === "/persona/targets") {
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(
        new Response(null, { status: 204, headers: h2 }),
        method,
        "/persona/targets",
        rawPath
      );
    }
    if (method === "POST" && path2 === "/persona/explainLine") {
      ...
      try {
        ...
        const h2 = withCors(new Headers(), origin);
        return addEngineHeaders(json(parsed, 200, h2), method, path2, rawPath);
      } catch (err) {
        const h2 = withCors(new Headers(), origin);
        return addEngineHeaders(
          json({ error: "openai_failed", message: String(err) }, 500, h2),
          method,
          path2,
          rawPath
        );
    ...
    if (method === "POST" && path2 === "/persona/logBias") {
      const body = await req.json();
      const topic = body.topic || "";
      const fields = body.fields || {};
      console.log("\u{1F9E0} [logBias] Multi-field mode (plans\u5BFE\u5FDC)", { topic, fields });
      try {
        const client = new OpenAI({ apiKey: env.OPENAI_API_KEY });
        const results = {};
...
- \u826F\u304F\u306A\u3044\u4E88\u60F3: ${p.bad || "\u2014"}`;
            ...
            if (n.plans?.length) {
              const planText = n.plans.map(
                (p, i) => `(${i + 1}) ${p.who || "\u8AB0\u304B"}\u306E\u8A08\u753B: \u4F55=${p.what || "\u2014"}\uFF0F\u3069\u3046\u3084\u3063\u3066=${p.how || "\u2014"}\uFF0F\u826F\u3044\u4E88\u60F3=${p.good || "\u2014"}\uFF0F\u60AA\u3044\u4E88\u60F3=${p.bad || "\u2014"}`
              ).join("\n");
              text2 += `
...
\u4EE5\u4E0B\u306E\u6587\u7AE0\u306F\u30EF\u30FC\u30AF\u30B7\u30FC\u30C8\u306E\u300C${field}\u300D\u6B04\u306B\u66F8\u304B\u308C\u305F\u5185\u5BB9\u3067\u3059\u3002

\u3042\u306A\u305F\u306E\u5F79\u5272\u306F\u3001\u4E3B\u89B3\u7684\u3067\u306F\u306A\u304F\u3001\u4E2D\u7ACB\u304B\u3064\u8AD6\u7406\u7684\u306A\u89B3\u5BDF\u8005\u3068\u3057\u30669\u3064\u306E\u89B3\u70B9\u304B\u3089\u6587\u7AE0\u3092\u5224\u5B9A\u3059\u308B\u3053\u3068\u3067\u3059\u3002

\u3010\u8D85\u91CD\u8981\u30EB\u30FC\u30EB\uFF08\u5FC5\u305A\u5B88\u308B\u3053\u3068\uFF09\u3011
- \u300C\u4E0D\u660E\u300D\u306F\u6700\u7D42\u624B\u6BB5\u3067\u3059\u3002\u672C\u5F53\u306B\u610F\u5473\u304C\u8AAD\u307F\u53D6\u308C\u306A\u3044\u6642\u3060\u3051\u4F7F\u7528\u3057\u3066\u304F\u3060\u3055\u3044\u3002
- \u6587\u306E\u9577\u3055\u30FB\u8A9E\u5F59\u306E\u5C11\u306A\u3055\u30FB\u7815\u3051\u305F\u6587\u4F53\u30FB\u7B87\u6761\u66F8\u304D\u30FB\u4E3B\u8A9E\u7701\u7565\u306A\u3069\u306F\u300C\u4E0D\u660E\u300D\u306E\u7406\u7531\u306B\u306A\u308A\u307E\u305B\u3093\u3002
- \u6587\u8108\uFF08\u524D\u63D0\u30FB\u56F0\u308A\u3054\u3068\u30FB\u539F\u56E0\u30FB\u5BFE\u7B56\u30FB\u8A08\u753B\u306A\u3069\u8907\u6570\u6B04\uFF09\u3092\u7DCF\u5408\u3057\u3066\u610F\u5473\u304C\u63A8\u6E2C\u3067\u304D\u308B\u5834\u5408\u306F\u3001\u4E0D\u660E\u3067\u306F\u306A\u304F\u300C\u5747\u8861\u300D\u3082\u3057\u304F\u306F\u6700\u3082\u8FD1\u3044\u89B3\u70B9\u3092\u8FD4\u3057\u3066\u304F\u3060\u3055\u3044\u3002
- \u7279\u306B\u300C\u56F0\u308A\u3054\u3068\u300D\u300C\u5177\u4F53\u4F8B\u300D\u300C\u539F\u56E0\u300D\u306A\u3069\u306F\u3001\u4E0D\u660E\u306B\u306A\u308A\u306B\u304F\u3044\u6B04\u3067\u3059\u3002\u610F\u5473\u304C\u7406\u89E3\u3067\u304D\u308B\u9650\u308A\u4E0D\u660E\u306B\u3057\u3066\u306F\u3044\u3051\u307E\u305B\u3093\u3002

\u8A55\u4FA1\u89B3\u70B9\uFF08\u5FC5\u305A\u3053\u306E9\u3064\u306E\u8A9E\u306E\u3044\u305A\u308C\u304B\u3067\u8FD4\u3059\uFF09\uFF1A
1. \u65AD\u5B9A
2. \u56E0\u679C\u304C\u7C97\u3044
3. \u6A29\u5A01\u306B\u4F9D\u5B58
4. \u4EEE\u8AAC\u904E\u591A(H\u226BE)
...
7. \u77DB\u76FE
8. \u5747\u8861
9. \u4E0D\u660E\uFF08\u203B\u4E0A\u8A18\u306E\u901A\u308A\u3001\u672C\u5F53\u306B\u8AAD\u89E3\u4E0D\u80FD\u306A\u5834\u5408\u306E\u307F\uFF09

\u3010\u5224\u5B9A\u30EB\u30FC\u30EB\u3011
- \u6587\u306E\u610F\u5473\u304C\u666E\u901A\u306B\u7406\u89E3\u3067\u304D\u308B\u5834\u5408 \u2192 \u4E0D\u660E\u306B\u306F\u3057\u306A\u3044\u3002
- \u5185\u5BB9\u304C\u3084\u3084\u62BD\u8C61\u7684\u3067\u3082 \u2192 \u4E0D\u660E\u306B\u306F\u3057\u306A\u3044\u3002
- \u5BA2\u89B3\u6027\u30FB\u56E0\u679C\u95A2\u4FC2\u30FB\u502B\u7406\u30FB\u4E3B\u89B3\u6027\u306A\u3069\u306E\u89B3\u70B9\u3067\u7279\u306B\u504F\u308A\u304C\u7121\u3051\u308C\u3070 \u2192 \u300C\u5747\u8861\u300D\u3002
- \u5224\u65AD\u6750\u6599\u304C\u5C11\u306A\u304F\u3066\u3082\u3001\u610F\u5473\u304C\u901A\u308B\u306A\u3089\u300C\u5747\u8861\u300D\u3002
- \u672C\u5F53\u306B\u6587\u7AE0\u306E\u610F\u5473\u304C\u5206\u304B\u3089\u306A\u3044\u5834\u5408\u3060\u3051\u300C\u4E0D\u660E\u300D\u3002
...
${text2}`
          ...
          const raw = completion.choices[0]?.message?.content || "{}";
          console.log(`\u{1F4E4} [${field}] response:`, raw);
          let parsed;
          try {
            parsed = JSON.parse(raw);
          } catch {
            parsed = { line: text2, flags: [], advice: "" };
          }
          if (!parsed.flags || parsed.flags.length === 0) {
            parsed.flags = ["\u5747\u8861"];
            parsed.advice = parsed.advice?.trim() || "\u30D0\u30E9\u30F3\u30B9\u826F\u304F\u66F8\u3051\u3066\u3044\u307E\u3059\u3002\u7279\u306B\u504F\u308A\u306F\u898B\u3089\u308C\u307E\u305B\u3093\u3002";
          }
          if (!Array.isArray(parsed.flags)) {
            parsed.flags = ["\u4E0D\u660E"];
            parsed.advice = "\u3053\u306E\u6587\u306E\u610F\u5473\u304C\u4E0D\u660E\u77AD\u3067\u3001\u8A55\u4FA1\u304C\u96E3\u3057\u3044\u3067\u3059\u3002";
          }
          const ORDER = [
            "\u65AD\u5B9A",
            "\u56E0\u679C\u304C\u7C97\u3044",
          ...
          parsed.flags.sort((a, b) => ORDER.indexOf(a) - ORDER.indexOf(b));
          results[field] = parsed;
        }
        const h2 = withCors(new Headers(), origin);
        return addEngineHeaders(json({ topic, results }, 200, h2), method, path2, rawPath);
      } catch (err) {
        const h2 = withCors(new Headers(), origin);
        return addEngineHeaders(
          json({ error: "openai_failed", message: String(err) }, 500, h2),
    ...
    if (method === "POST" && path2 === "/persona/noise") {
      const body = await req.json();
      const topic = body.topic || "";
      const note = body.note || {};
      const textForAI = Array.isArray(note.plans) ? note.plans.map((p, i) => {
        return `(${i + 1}) ${p.who || note.author || "\u8AB0\u304B"}\u306E\u8A08\u753B:
- \u4F55\u3092: ${p.what || "\u2014"}
- \u3069\u3046\u3084\u3063\u3066: ${p.how || "\u2014"}
- \u826F\u3044\u4E88\u60F3: ${p.good || "\u2014"}
- \u826F\u304F\u306A\u3044\u4E88\u60F3: ${p.bad || "\u2014"}`;
...
${textForAI}
        ...
        const h2 = withCors(new Headers(), origin);
        return addEngineHeaders(
          json({ ok: true, perspectives: parsed, source: note }, 200, h2),
          method,
          path2,
          rawPath
        );
    ...
    if (method === "POST" && path2 === "/persona/arrangeBoard") {
      const body = await req.json();
      const boards = Array.isArray(body.boards) ? body.boards : [{ topic: body.topic || "", spec: body.spec || "impact-feasibility", notes: body.notes || [] }];
      try {
        const client = new OpenAI({ apiKey: env.OPENAI_API_KEY });
        const results = [];
...
${JSON.stringify(b.notes, null, 2)}`
        ...
        const h2 = withCors(new Headers(), origin);
        return addEngineHeaders(json({ ok: true, results }, 200, h2), method, path2, rawPath);
      } catch (err) {
    ...
    if (method === "POST" && path2 === "/persona/evidenceQuest") {
      const { topic = "", teamName: teamName2 = "", notes = [] } = await req.json();
      try {
        const client = new OpenAI({ apiKey: env.OPENAI_API_KEY });
...
${normalizedNotes.map(
    ...
    if (method === "OPTIONS" && path2 === "/persona/teamState") {
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(
        new Response(null, { status: 204, headers: h2 }),
        method,
    ...
    if (method === "POST" && path2 === "/persona/teamState") {
      const body = await req.json();
      const team = body.team || "";
      const role = body.role || "user";
      const userId = body.userId || "";
      if (!team) return json({ error: "missing_team" }, 400);
      const current = await env.TEAM_STATE.get(team, "json") || {};
      if (role === "leader") {
        const payload = {
          topic: body.topic !== void 0 ? body.topic : current.topic || "",
          target: body.target !== void 0 ? body.target : current.target || "",
          scenario: body.scenario !== void 0 ? body.scenario : current.scenario || "",
          premise: body.premise !== void 0 ? body.premise : current.premise || "",
          trouble: body.trouble !== void 0 ? body.trouble : current.trouble || "",
          otherPrem: body.otherPrem !== void 0 ? body.otherPrem : current.otherPrem || "",
          cause: body.cause !== void 0 ? body.cause : current.cause || "",
          idea: body.idea !== void 0 ? body.idea : current.idea || "",
          plans: body.plans !== void 0 ? body.plans : current.plans || [],
          updatedAt: (/* @__PURE__ */ new Date()).toISOString()
        };
        await env.TEAM_STATE.put(team, JSON.stringify(payload));
      } else if (role === "user") {
        const currentPlans = Array.isArray(current.plans) ? current.plans : [];
        const userPlans = Array.isArray(body.plans) ? body.plans : [];
        const mergedPlans = [
          ...currentPlans.filter((p) => p.userId && p.userId !== userId),
          ...userPlans.map((p) => ({ ...p, userId }))
        ];
        const merged = {
          ...current,
          plans: mergedPlans,
        ...
        await env.TEAM_STATE.put(team, JSON.stringify(merged));
      } else {
        return json({ ok: false, note: "read-only role" }, 200);
      }
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(json({ ok: true, role }, 200, h2), method, path2, rawPath);
    }
    if (method === "GET" && path2 === "/persona/teamState") {
      const h2 = withCors(new Headers(), origin);
      try {
    ...
    if (method === "POST" && path2 === "/persona/subscribe") {
      const body = await req.json();
      const user = body.user || "anonymous";
      const h2 = withCors(new Headers(), origin);
      const ADMIN_IDS = ["yamane", "admin", "teacher"];
      if (ADMIN_IDS.includes(user)) {
        await env.TEAM_STATE.put(user, JSON.stringify({
          plan: "premium",
          admin: true,
      ...
      try {
        const stripeRes = await fetch("https://api.stripe.com/v1/checkout/sessions", {
          ...
          body: new URLSearchParams({
            ...
            "line_items[0][price_data][product_data][name]": "\u601D\u8003\u30A2\u30B9\u30EC\u30C1\u30C3\u30AF \u30D7\u30EC\u30DF\u30A2\u30E0\u30D7\u30E9\u30F3",
            "line_items[0][price_data][unit_amount]": "500",
            "line_items[0][price_data][recurring][interval]": "month",
            "line_items[0][quantity]": "1",
            success_url: "https://shiko-athletic-test.pages.dev/success",
            cancel_url: "https://shiko-athletic-test.pages.dev/cancel",
            // 🔥 Stripe → webhook へ渡す変数
            "metadata[user]": user
          })
        });
        const resultText = await stripeRes.text();
        let result;
        try {
    ...
    if (method === "POST" && path2 === "/persona/stripeWebhook") {
      ...
      try {
        ...
        console.log("\u{1F4E9} Webhook event:", event.type);
        console.log("\u{1F4E9} Webhook metadata.user =", event.data.object?.metadata?.user);
        if (event.type === "checkout.session.completed" || event.type === "checkout.session.async_payment_succeeded") {
          ...
          if (user) {
            ...
            console.log("\u2705 Stripe success \u2014 Saved Premium:", user);
          } else {
            console.warn("\u26A0\uFE0F metadata.user \u304C\u7121\u3044");
          }
        }
    ...
    if (method === "GET" && path2 === "/persona/getPlan") {
      const user = url.searchParams.get("user") || "anonymous";
      const data = await env.TEAM_STATE.get(user, "json");
      const h2 = withCors(new Headers(), origin);
      return new Response(JSON.stringify({
        user,
        plan: data?.plan || "free",
    ...
    if (method === "OPTIONS" && path2 === "/persona/login-request") {
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(
        new Response(null, { status: 204, headers: h2 }),
        method,
    ...
    if (method === "POST" && path2 === "/persona/login-request") {
      const body = await req.json();
      const redirect = body.redirect || "https://shiko-athletic-test.pages.dev/";
      const clientId = env.GOOGLE_CLIENT_ID;
      const callbackUrl = "https://ms-engine-test.sinnosukeyamane.workers.dev/persona/oauth-callback";
      ...
      const url2 = "https://accounts.google.com/o/oauth2/v2/auth?" + new URLSearchParams({
        client_id: clientId,
    ...
    if (method === "GET" && path2 === "/persona/oauth-callback") {
      const code = url.searchParams.get("code") || "";
      const state = url.searchParams.get("state") || "";
      const record = await env.TEAM_STATE.get(`oauth_state_${state}`, "json");
      if (!record) return text("Invalid state", 400);
      ...
      const tokenRes = await fetch("https://oauth2.googleapis.com/token", {
        ...
        body: new URLSearchParams({
          ...
          redirect_uri: "https://ms-engine-test.sinnosukeyamane.workers.dev/persona/oauth-callback",
          grant_type: "authorization_code"
        })
      });
      const tokenData = await tokenRes.json();
      const idToken = tokenData.id_token;
      const payload = decodeJwtPayload(idToken.split(".")[1]);
      const email = payload.email;
      const name = payload.name;
      const picture = payload.picture;
      const userKey = `user_${email}`;
      await env.TEAM_STATE.put(userKey, JSON.stringify({ email, name, picture }));
      return Response.redirect(`${redirect}?token=${userKey}`, 302);
    }
    if (method === "GET" && path2 === "/export/pdf") {
      const team = new URL(req.url).searchParams.get("team");
      if (!team) {
        return new Response("team is required", { status: 400 });
      }
      const raw = await env.TEAM_STATE.get(`state:${team}`);
      if (!raw) {
      ...
      let y = 800;
      const lineHeight = 18;
      const draw = /* @__PURE__ */ __name((text2) => {
        page.drawText(text2, { x: 40, y, size: 11, font });
        y -= lineHeight;
      }, "draw");
      draw(`\u601D\u8003\u30A2\u30B9\u30EC\u30C1\u30C3\u30AF \u5B9F\u884C\u30ED\u30B0`);
      draw(`\u30C1\u30FC\u30E0: ${data.team}`);
      draw(`\u4F5C\u6210\u65E5\u6642: ${(/* @__PURE__ */ new Date()).toLocaleString()}`);
      y -= lineHeight;
      for (const [key, value] of Object.entries(data)) {
        draw(`${key}:`);
        draw(typeof value === "string" ? value : JSON.stringify(value, null, 2));
        y -= lineHeight;
      }
      ...
      return new Response(pdfBytes, {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="${team}.pdf"`
        }
      });
    }
    if (method === "GET" && path2 === "/persona/getUser") {
      const user = url.searchParams.get("user") || "";
      if (!user) {
        const h3 = withCors(new Headers(), origin);
        return addEngineHeaders(
          json({ error: "missing_user" }, 400, h3),
          method,
          path2,
          rawPath
        );
      }
      const record = await env.TEAM_STATE.get(user, "json");
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(
        json(record || { error: "not_found" }, record ? 200 : 404, h2),
        method,
        path2,
        rawPath
      );
    ...
    return addEngineHeaders(json({ error: "not_found", path: rawPath }, 404, h), method, path2, rawPath);
  }
};
export {
