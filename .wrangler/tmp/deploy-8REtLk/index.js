...
var index_default = {
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === "GET" && pathname === "/persona/checkKey") {
      const h2 = withCors(origin);
      const exists = !!env.OPENAI_API_KEY;
      const masked = exists ? env.OPENAI_API_KEY.slice(0, 5) + "****" + env.OPENAI_API_KEY.slice(-4) : "(none)";
      const res = json(
        {
          exists,
          maskedKey: masked
      ...
      return addEngineHeaders(
        corsResponse(await res.text(), 200, h2, origin),
        method,
        pathname
      );
    }
...
${normalizedNotes.map(
    ...
    if (method === "GET" && pathname === "/persona/teamState") {
      const h2 = withCors(new Headers(), origin);
      try {
        const team = url.searchParams.get("team");
        if (!team) {
          const res2 = json({ error: "missing_team" }, 400, h2);
          return corsResponse(await res2.text(), 400, h2, origin);
        }
        ...
        const res = json(data, 200, h2);
        return corsResponse(await res.text(), 200, h2, origin);
      } catch (err) {
        const res = json({ error: "teamState_get_failed", message: String(err) }, 500, h2);
        return corsResponse(await res.text(), 500, h2, origin);
      }
    }
