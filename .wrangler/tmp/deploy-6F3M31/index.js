...
\u51FA\u529B\u306FJSON\u914D\u5217\u306E\u307F\u3067\u3001\u65E5\u672C\u8A9E\u306E\u540D\u8A5E\u53E5\u30678\u301C12\u4EF6\u7A0B\u5EA6\u3002
    ...
    if (url.pathname === "/auth/companyLogin" && request.method === "POST") {
      const { companyCode, password } = await request.json();
      ...
      if (!record || record.enabled !== true) {
        return json({ ok: false, error: "INVALID_COMPANY" }, 401);
      }
...
${normalizedNotes.map(
    ...
    if (method === "DELETE" && path2 === "/persona/teamState") {
      ...
      console.log(`\u{1F5D1} team deleted: ${team}, keys=${list.keys.length}`);
      return addEngineHeaders(
        json(
          {
            ok: true,
    ...
    if (method === "POST" && path2 === "/team/updateMembers") {
      ...
      try {
        ...
        for (const k of list.keys) {
          const userId = k.name.replace(`team:${team}:user:`, "");
