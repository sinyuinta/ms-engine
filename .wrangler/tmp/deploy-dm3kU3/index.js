...
var index_default = {
  ...
  async fetch(request, env, ctx) {
    ...
    if (method === "POST" && path2 === "/persona/userState") {
      const body = await req.json();
      const { team, userId } = body;
      if (!team || !userId) {
        return json({ error: "missing_team_or_user" }, 400);
      }
      const key = `user_state:${team}:${userId}`;
      const payload = {
        team,
        userId,
        author: body.author || "\u2014",
      ...
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(json({ ok: true }), method, path2, rawPath);
    }
    if (method === "GET" && path2 === "/persona/teamUserStates") {
      const team = url.searchParams.get("team");
      if (!team) return json({ error: "missing_team" }, 400);
      const list = await env.TEAM_STATE.list({
        prefix: `user_state:${team}:`
      });
      const users = [];
      for (const k of list.keys) {
        const raw = await env.TEAM_STATE.get(k.name);
        if (raw) users.push(JSON.parse(raw));
      }
      const h2 = withCors(new Headers(), origin);
      return addEngineHeaders(json({ users }), method, path2, rawPath);
    }
...
${normalizedNotes.map(
    ...
    if (method === "POST" && path2 === "/persona/teamState") {
      ...
      if (role === "leader") {
        const payload = {
          ...
          scenario: body.scenario ?? current.scenario ?? "",
          premise: body.premise ?? current.premise ?? "",
          trouble: body.trouble ?? current.trouble ?? "",
          otherPrem: body.otherPrem ?? current.otherPrem ?? "",
          cause: body.cause ?? current.cause ?? "",
          idea: body.idea ?? current.idea ?? "",
      ...
      const logs = await env.TEAM_STATE.get(logKey, "json") || [];
      logs.push({
        team,
        role,
        userId,
    ...
    if (method === "POST" && path2 === "/persona/login-request") {
      ...
      const callbackUrl = "https://ms-engine-test.s-yamane.workers.dev/persona/oauth-callback";
    ...
    if (method === "GET" && path2 === "/persona/oauth-callback") {
      ...
      const tokenRes = await fetch("https://oauth2.googleapis.com/token", {
        ...
        body: new URLSearchParams({
          ...
          redirect_uri: "https://ms-engine-test.s-yamane.workers.dev/persona/oauth-callback",
